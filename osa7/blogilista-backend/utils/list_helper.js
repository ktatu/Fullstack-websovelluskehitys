...
const favoriteBlog = (blogs) => {
    ...
    for (let blog of blogs) {
        blogWithMostLikes =
            blog.likes < blogWithMostLikes.likes ? blogWithMostLikes : blog
    }
    return blogWithMostLikes.likes === -1 ? undefined : blogWithMostLikes
}

const mostBlogs = (blogs) => {
    let authorsAndBlogCounts = _.map(
        _.countBy(blogs, "author"),
        (val, key) => ({ author: key, blogs: val })
    )

    let authorWithMostBlogs = _.head(
        _.orderBy(authorsAndBlogCounts, ["blogs"], ["desc"])
    )
