...
describe("Blog app", function () {
    ...
    describe("When logged in", function () {
        ...
        it("A blog can be created", function () {
            ...
            cy.get("#url-input").type(cypressBlog.url)

            cy.get("#author-input").type(cypressBlog.author)

            cy.get("#title-input").type(cypressBlog.title)
        ...
        it("A blog can be deleted", function () {
            ...
            cy.get(".expandedViewButton").click()

            cy.contains("delete").click()

            cy.contains(cypressBlog.title).should("not.exist")
        })
        ...
        it.only("Blogs are sorted in order of likes", function () {
            ...
            cy.get(".expandedViewButton").eq(0).click()
            ...
            cy.get(".blog-title").eq(0).contains(cypressBlog.title)

            cy.likeBlog(secondBlog.title).wait(1000)
            cy.likeBlog(secondBlog.title).wait(1000)
            cy.likeBlog(secondBlog.title).wait(1000)

            cy.get(".blog-title").eq(0).contains(secondBlog.title)
        })
    })
})
